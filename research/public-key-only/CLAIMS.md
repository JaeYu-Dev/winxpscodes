...
| C-015 | Within one shipped-SP3 RSAENH CGR64 call, the second SystemFunction036 input has no independent stack-prehistory root | CONFIRMED MACHINE-DATAFLOW | same D640 frame/local is reused; removes one nominal 160-bit root per 64-byte call |
...
| C-025 | Provenance of the first relevant SystemFunction036 pre-call buffers collapses below their remaining nominal independent widths | OPEN | first-round last-writer/support theorem remains high-value |
...
| C-032 | The joint reachable image of `M1..M6` factors as six independent kernel secrets | FALSIFIED BY V24 RATCHET | all consumed materials descend from one persistent 80-byte KSecDD state trajectory plus sequential gather inputs |
| C-033 | KSecDD long-path rekey materials satisfy a persistent-state recurrence `X_{t+1}=VLH(X_t,P_t)` and returned material is a deterministic RC4 transform keyed by the new 80-byte state | CONFIRMED SP3 TRACE + REPLAY | replaces six-root model with one ratchet plus fresh gather differentials |
| C-034 | V24 post-gather captures contain candidate kernel pointers `e1337008,e1379008,e137f008,e138e008,e12cc008,e134d008,e1379008,e137e008`, with cycle 2 and 7 repeating the same value | CONFIRMED OBSERVATION | allocator-reuse candidate; no entropy conclusion by itself |
| C-035 | The repeated V24 `0xe1379008` value is proven to be the KSecDD 0xE00 `pbWorkingBuffer` address | OPEN | required before allocator-address reuse can enter a theorem |
| C-036 | Same-address cycle-2/cycle-7 reuse proves residual hashed bytes are SELF with zero fresh freedom | OPEN / DO NOT INFER | requires exact working-buffer identity and per-byte no-intervening-writer proof |
| C-037 | The KSecDD allocation capacity 0xE00 should be counted as the hashed RNG input width | FALSIFIED | target replay hashes the used 0x258-byte pool; avoid 2984-byte overcount |
| C-038 | In the pinned long-gather campaign, the direct allocation-history region before the first QSI contribution is narrowed to 24 bytes | CONFIRMED RECOVERED ANALYSIS | shrinks the direct unassigned provenance region; 24 bytes must not be equated with 192 bits of entropy |
| C-039 | Class-05 SystemProcessInformation contributes a captured 0xdc8=3528-byte region that is SHA-1 projected to one 20-byte downstream value | CONFIRMED TRACE/REPLAY | local class-05 boundary has image <=2^160 regardless of raw byte width |
| C-040 | In the pinned class-05 capture, exactly the first 0x1b8 bytes are written and the remaining ~3 KB are untouched pool tail | OPEN | highly valuable if proved; exact XP SP3 ExpGetProcessInformation cold blocks still required |
| C-041 | Existing 80-byte registry Seed is read into the SP3 kernel RNG state; universal `X0=0` from a ReadSeed failure is false | CONFIRMED EXACT-SP3 RECOVERED WORK | zero state is a missing-Seed branch, so persistent X0 ancestry remains a blocker |

## Rule
No target-SP3 claim moves to CONFIRMED from source inspection alone unless independently anchored by exact binary/trace/replay evidence. Correlation is only promoted when it removes an actual data-flow ancestor or produces a formal search/image reduction. A local <=2^128 projection is not by itself a Bitcoin private-key break unless the downstream joint search is also below generic ECDLP. Chronological RNG events are never counted as independent roots without a data-flow proof that they survive into `d_first`. Repeated allocator addresses do not imply preserved residual contents without exact working-buffer identity and last-writer provenance. Byte width is never converted directly into entropy bits without a support/provenance proof.
