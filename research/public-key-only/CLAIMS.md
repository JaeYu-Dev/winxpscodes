...
| C-025 | Provenance of SystemFunction036 pre-call buffers L1/L2 collapses below two independent 160-bit roots | OPEN | high-value reachability target |
...
| C-034 | V24 post-gather captures contain candidate kernel pointers `e1337008,e1379008,e137f008,e138e008,e12cc008,e134d008,e1379008,e137e008`, with cycle 2 and 7 repeating the same value | CONFIRMED OBSERVATION | strong allocator-reuse candidate; no entropy conclusion yet |
| C-035 | The repeated V24 `0xe1379008` value is proven to be the KSecDD 0xE00 `pbWorkingBuffer` address | OPEN | required before allocator provenance can enter the key-image theorem |
| C-036 | Same-address cycle-2/cycle-7 reuse proves residual hashed bytes are SELF with zero fresh freedom | OPEN / DO NOT INFER | requires per-byte no-intervening-writer proof |
| C-037 | The KSecDD allocation capacity 0xE00 should be counted as the hashed RNG input width | FALSIFIED | target trace hashes only the used 0x258-byte prefix; avoid 2984-byte overcount |
